<template>
  <div class="admin-disability-info">
    <el-card>
      <template #header>
      ...
      <el-table :data="recordList" v-loading="loading" border stripe>
        ...
        <el-table-column label="残疾类型" width="120" align="center">
          <template #default="{ row }">{{ disabilityTypeMap[row.disabilityType] || '未知' }}</template>
        </el-table-column>
        <el-table-column label="残疾人证号" width="180" show-overflow-tooltip>
          <template #default="{ row }">{{ row.certificateNo || '-' }}</template>
        </el-table-column>
        <el-table-column label="证件扫描件" width="100" align="center">
          <template #default="{ row }">
            <el-button v-if="row.certificateFileId" link type="primary" size="small" @click="previewFile(row.certificateFileId)">查看</el-button>
      ...
      <el-empty v-if="!loading && recordList.length === 0" description="暂无残疾信息记录" style="margin-top: 40px" />
    </el-card>

    <!-- 审核对话框 -->
    <el-dialog v-model="auditDialogVisible" title="审核残疾信息" width="480px" destroy-on-close>
      <el-form :model="auditForm" label-width="100px">
        <el-form-item label="学员姓名">
          <span>{{ currentRow?.studentName }}</span>
        </el-form-item>
        <el-form-item label="残疾类型">
          <span>{{ currentRow ? (disabilityTypeMap[currentRow.disabilityType] || '未知') : '' }}</span>
        </el-form-item>
        <el-form-item label="残疾人证号">
          <span>{{ currentRow?.certificateNo }}</span>
        </el-form-item>
        <el-form-item label="证件扫描件">
          <el-button v-if="currentRow?.certificateFileId" link type="primary" size="small" @click="previewFile(currentRow.certificateFileId)">查看</el-button>
          <span v-else class="text-gray">-</span>
        </el-form-item>
...
import { getDisabilityInfoPage, auditDisabilityInfo } from '@/api/disabilityInfo'
...
async function fetchList() {
  ...
  try {
    ...
    const res = await getDisabilityInfoPage(params)
    recordList.value = res.records || []
    total.value = res.total || 0
  } catch (error) {
    console.error('获取残疾信息失败:', error)
    recordList.value = []
  } finally {
    loading.value = false
  }
...
async function handleAuditSubmit() {
  ...
  try {
    await auditDisabilityInfo(currentRow.value.id, auditForm.auditStatus, auditForm.auditRemark)
